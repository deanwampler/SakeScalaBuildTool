...
object ResultSpec extends Specification {
    ...
    "ResultString.asString method" should {
        "formats a user-friendly message" in {
            val p1 = new Passed()
            ResultString.asString(p1.result, p1.message) must be_==("(status: ?)")
            val p2 = new Passed(None)
            ResultString.asString(p2.result, p2.message) must be_==("(status: ?)")
            val p3 = new Passed(None, None)
            ResultString.asString(p3.result, p3.message) must be_==("(status: ?)")
            val p4 = new Passed(Some(0))
            ResultString.asString(p4.result, p4.message) must be_==("(status: 0)")
            val p5 = new Passed(Some(0), None)
            ResultString.asString(p5.result, p5.message) must be_==("(status: 0)")
            val p6 = new Passed(Some(0), Some("message1"))
            ResultString.asString(p6.result, p6.message) must be_==("(status: 0, message: message1)")

            val f1 = new Failed()
            ResultString.asString(f1.result, f1.message) must be_==("(status: ?)")
            val f2 = new Failed(None)
            ResultString.asString(f2.result, f2.message) must be_==("(status: ?)")
            val f3 = new Failed(None, None)
            ResultString.asString(f3.result, f3.message) must be_==("(status: ?)")
            val f4 = new Failed(Some(1))
            ResultString.asString(f4.result, f4.message) must be_==("(status: 1)")
            val f5 = new Failed(Some(1), None)
            ResultString.asString(f5.result, f5.message) must be_==("(status: 1)")
            val f6 = new Failed(Some(1), Some("message1"))
            ResultString.asString(f6.result, f6.message) must be_==("(status: 1, message: message1)")
        }
    }
