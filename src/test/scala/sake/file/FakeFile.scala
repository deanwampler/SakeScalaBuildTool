...
object FakeFileForSpecs {
  val oneFakeFile = new FilesFinder() {
    ...
    def toFile(path: String): File = path match {
      case "." => new FakeFileForSpecs(path, true, true, Seq("foo"))
      case "foo" => new FakeFileForSpecs(path, true, true, Seq("bar1"))
      case "foo/bar1" => new FakeFileForSpecs(path, true, true, Seq("ASpec.class"))
      case "foo/bar1/ASpec.class" => new FakeFileForSpecs(path, true, false, Vector.empty[String])
    }
  }
